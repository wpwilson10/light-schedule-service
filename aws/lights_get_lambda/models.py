...
class LightConfig:
    ...
    def __init__(
        ...
        self.mode: Literal["dayNight", "scheduled", "demo"] = mode
        ...
        self.update_time_unix: int = convert_to_tomorrow_unix_timestamp(
            self.UPDATE_TIME, 0
        )  # Initialize with UTC
    ...
    @classmethod
    def from_dict(cls, data: Any) -> "LightConfig":
        """Creates a LightConfig instance from a dictionary."""
        if not data:
            return cls.create_empty()

        mode: str = data.get("mode", cls.DEFAULT_MODE)
        schedule_data: list[ScheduleItem] = data.get("schedule", [])

        config = cls(
            cast(Literal["dayNight", "scheduled", "demo"], mode),
            schedule_data,
        )
    ...
    def to_dict(
        self,
    ) -> dict[str, str | int | list[ScheduleItem] | ScheduleItem | None]:
    ...
    def update_schedule_times(self, timezone_offset: int) -> None:
        ...
        for item in self.schedule:
            if "time" in item:
                item["unix_time"] = convert_to_unix_timestamp(
                    item["time"], timezone_offset
                )
    ...
    def update_sleep_times(self, timezone_offset: int) -> None:
        ...
        if not self.bed_time:
            ...
            self.bed_time["unix_time"] = convert_to_unix_timestamp(
                self.bed_time["time"], timezone_offset
            )

        if not self.night_time:
            self.night_time = ScheduleItem(
                time=DEFAULT_NIGHT_TIME,
                unix_time=convert_to_unix_timestamp(
                    DEFAULT_NIGHT_TIME, timezone_offset
                ),
            ...
            self.night_time["unix_time"] = convert_to_unix_timestamp(
                self.night_time["time"], timezone_offset
            )
    ...
    def __adjust_twilight_end(self, sunset_time: str) -> str:
        ...
        total_minutes = (
            sunset_parts[0] * 60 + sunset_parts[1] + self.TWILIGHT_END_OFFSET
        )
    ...
    def update_daylight_times(
        ...
        self.natural_sunset = self.__create_or_update_schedule_item(
            self.natural_sunset, sunset, timezone_offset, *DaylightBrightness.SUNSET
        )
        self.natural_twilight_end = self.__create_or_update_schedule_item(
            self.natural_twilight_end,
            twilight_end,
            timezone_offset,
    ...
    def update_next_update_time(self, timezone_offset: int) -> None:
        ...
        self.update_time_unix = convert_to_tomorrow_unix_timestamp(
            self.UPDATE_TIME, timezone_offset
        )
